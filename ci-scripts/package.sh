...
OUT_FILE=checkpointer.$EXT
OUT_PATH=$ARTIFACTS_DIR/checkpointer.$EXT
...
fpm --fpm-options-file $CONFIG_FILE -C $BIN_DIR -v $PKG_VERSION -p $OUT_PATH checkpointer=$INSTALL_DIR/checkpointer

echo "Compressing package for "$TARGET
tar -cvzf checkpointer_$TARGET.tar.gz -C $ARTIFACTS_DIR $OUT_FILE
